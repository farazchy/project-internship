...
# Group by destinationAirport and get the minimum (cheapest) fare along with the airport providing the cheapest fare
cheapest_flights = df_cleaned.groupBy("destinationairport", "startingairport") \
    .agg(
        F.min("totalFare").alias("cheapest_fare")
    ) \
    .withColumn("cheapest_fare", F.col("cheapest_fare")) \
    .orderBy("cheapest_fare")

# Show the result with destination airport, starting airport, and the cheapest fare
cheapest_flights.show(10)